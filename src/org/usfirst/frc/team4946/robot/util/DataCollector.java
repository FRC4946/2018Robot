...
public class DataCollector {
	...
	public static void writeSecData() throws IOException {
		...
		m_velList.add(0.0); //Add the initial left and right velocities, accelerations, and jerks
		m_velList.add(0.0);
		m_accelList.add(0.0);
		m_accelList.add(0.0);
		m_jerkList.add(0.0);
		...
		while(m_dataReader.hasNextLine()) {
			...
			//[0]Raw Count from left encoder
			//[1]Raw Count from right encoder
			//[2]Recorded time in seconds
			//[3]Left encoder distance
			//[4]Right encoder distance
			
			m_deltaTime = Double.parseDouble(m_secondValueArray[2]) - Double.parseDouble(m_firstValueArray[2]);
					
			m_velList.add((Double.parseDouble(m_secondValueArray[3]) - Double.parseDouble(m_firstValueArray[3]))/m_deltaTime); //Left
